...
def test_admin_distribute_report_and_attacker_check_report_status():
    ...
    stdin, stdout, stderr = s.exec_command('cd gongfangautotest/chengsheng;'
                                           'pyresttest ' + config.remotehost + ' 10_new_admin_distribute_'
                                                                               'report_victim_check-report'
                                                                               '-status.yaml --skip_term_colors')
    out = stdout.readlines()
    print(out)
    check = ['Test Group admin-distribute-victim-report-and-check-report-status'
